...
class Wire():
    ...
    def __init__(self, points: np.ndarray[np.float64],
        ...
        if material == None and cross_sectional_area == -1.0:
            ...
            self.mass = length * cross_sectional_area * MATERIAL_DENSITIES[material]
            self.resistance = MATERIAL_RESISTIVITIES[material] * length / cross_sectional_area

    def get_wire_point(self) -> np.array[np.float64]:
        pass
    ...
    def get_electromotive_force(self, particles: list[PointParticle], electric_field: np.ndarray[np.float64]) -> np.float64:
        ...
        emf = -scipy.integrate.quad(
            lambda l: np.dot(
                sum_electric_fields(self.points[0] + wire_vector_hat * l),
                wire_vector_hat
            ),
            0,
            self.get_length()
