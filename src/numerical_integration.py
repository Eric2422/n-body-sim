...
class NumericalIntegration:
    ...
    def eulers(
        ...
        for i in range(num_time_steps):
            ...
            try:
                # Allow for integrating backwards.
    ...
    def runge_kutta(
        ...
        """Approximate the solution to the IVP via the classic Runge-Kutta method
        using the given time step size and number of time steps.

        For more details about the classic Runge-Kutta method,
        see [Runge–Kutta methods#The Runge-Kutta method](https://en.wikipedia.org/wiki/Runge-Kutta_methods#The_Runge–Kutta_method).
        ...
        if time_step_size == 0:
            raise ValueError(
                "The Runge-Kutta method requires a non-zero time step size.")

        # Prevent an invalid number of time steps.
        if num_time_steps < 1:
            raise ValueError(
                "The Runge-Kutta method requires at least one time step.")
        ...
        for i in range(num_time_steps):
            ...
            try:
                # Allow for integrating backwards.
                values += abs(time_step_size / 6) * (k1 + 2 * k2 + 2 * k3 + k4)
